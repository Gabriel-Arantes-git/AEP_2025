...
type inputPattern = {descricao: string}
@Component({
...
export class DropdownOpcoesComponent<T extends inputPattern> implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if(changes['existingOption']){
      this.inputValue = this.existingOption ? this.existingOption.descricao : '';
    }
  }
  ...
  filter(event: Event) {
    ...
    this.filteredOptions = this.options.filter(o => o.descricao.toLowerCase().includes(valor));
